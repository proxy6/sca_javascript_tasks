...
let inventory = [
  ...
  let lastCar = (`The last car in the inventory is a ${inventory[inventory.length-1].car_year} ${inventory[inventory.length-1].car_make} ${inventory[inventory.length-1].car_model}`);
  console.log(lastCar);
  ...
  let BMWAndAudi = [];
  for(let i=0; i<inventory.length-1; i++){
    // check for BMW and AUdi cars
    if(((inventory[i].car_make) === "BMW") || ((inventory[i].car_make) === "Audi")){
      //create an object assign the values to the object. Then push it into the array:
      var newCarObj = {};
      newCarObj[inventory[i].id]= inventory[i].car_make;
      newCarObj[inventory[i].id]= inventory[i].car_model;
      newCarObj[inventory[i].id]= inventory[i].car_year;
      //push obeject into BMWAndAudi variable
      BMWAndAudi.push(newCarObj);
      }
    }
