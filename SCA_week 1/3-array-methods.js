...
const runners = [
  ...
  runners.forEach(element => {
       //create an object assign the values to the object. Then push it into the array:
    let newObj ={};
    newObj.first = (element.first_name);
    newObj. last = (element.last_name);
    //push newObj into fullNames variable
    fullNames.push(newObj);
  });
  ...
  // Problem 1
  //The Event Director wants to send a thank you email to all the invited guest for their donations and needs their email address only. Using .map() to populate an array with just the email addresses
  let emailAddress = [];
  emailAddress = runners.map(element=> element.email);
  console.log(emailAddress);

  // Problem 2
  //The event director needs to group the runners according to their sizes of shirt "XS", "S", "M", "L", "XL", "2XL" and "3XL". this is to help in the distribution of the shirt and to facilitate the ordering
  let XS = [];
  let S = [];
  let M = [];
  let L = [];
  ...
  XS = runners.filter(element=>element.shirt_size === "XS")
  S = runners.filter(element=>element.shirt_size === "S")
  M = runners.filter(element=>element.shirt_size === "M")
  L = runners.filter(element=>element.shirt_size === "L")
  XL = runners.filter(element=>element.shirt_size === "XL")
  _2XL = runners.filter(element=>element.shirt_size === "2XL")
  _3XL = runners.filter(element=>element.shirt_size === "3XL")

  //uncomment the size you want to print to the console
  console.log(XS);
        //number of XS small to be ordered
            console.log(XS.length);
  //console.log(S);
        //number of S small to be ordered
             //console.log(S.length);

 // console.log(M);
        //number of M small to be ordered
             //console.log(M.length);

  //console.log(L);
        //number of L small to be ordered
          //console.log(L.length);
          
  //console.log(XL);
        //number of XL small to be ordered
          //console.log(XL.length);

  //console.log(_2XL);
        //number of 2XL small to be ordered
          // console.log(_2XL.length);

 // console.log(_3XL);
        //number of 3XL small to be ordered
           //console.log(_3XL.length);


  // Problem 3
  //The Event Director needs the names of the companies that donated. using forEach function to get the names.
  let companyName = [];
  runners.forEach(element=>{
